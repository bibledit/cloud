  ...
  if (webserver_request.post_count("submit")) {
    ...
    if (form_is_valid) {
      ...
      if (webserver_request.session_logic()->attempt_login (user, pass, touch_enabled)) {
        // Log the login.
        database::logs::logv1 (webserver_request.session_logic ()->get_username () + " logged in");
        ...
        database::logs::logv1 ("Failed login attempt for user " + user + " with password " + pass, roles::admin);
      }
    }
  }
