...
void bible_logic::import_resource(const std::string& bible, const std::string& resource)
{
    ...
    for (const auto book : database_versifications.getMaximumBooks())
    {
        const std::string bookName = database::books::get_english_from_id(static_cast<book_id>(book));

        for (const auto chapter : database_versifications.getMaximumChapters(book))
        {
...
std::string bible_logic::unsent_unreceived_data_warning()
{
    ...
    if (data_time)
    {
        ...
        if (now > data_time)
        {
            warning.clear();
            warning.append(
                translate("It has been some time ago that changes in the Bible text were received from the Cloud."));
            warning.append(" ");
    ...
    if (data_time)
    {
        ...
        {
            warning.clear();
            warning.append(translate(
                "There are pending changes in the Bible text that have not yet been sent to the Cloud for some time."));
            warning.append(" ");
            warning.append(translate("This will right itself in a little time if you are connected to the Internet."));
            warning.append(" ");
            warning.append(translate("If not please do a Send/receive action."));
                                          ...
                                          const std::vector<Merge_Conflict>& conflicts)
{
    ...
    for (const auto& conflict : conflicts)
    {
        ...
        if (conflict.book) newsubject.append(
            " | " + filter_passage_display(conflict.book, conflict.chapter, std::string()));
        ...
        for (const auto verse : filter::usfm::get_verse_numbers(conflict.result))
        {
        ...
        node.text().set(
            "You sent changes to the Cloud. The changes were merged with other changes already in the Cloud. The crossed out parts below could not be merged. They were replaced with the bold text below. You may want to check the changes or resend them.");
                                              ...
                                              const std::string& oldusfm, const std::string& newusfm)
{
    ...
    std::string information{};
    information.append(
        translate("While sending Bible text to Bibledit Cloud, you did not have write access to this chapter."));
    information.append(" ");
                                          ...
                                          std::vector<std::string>& content_out)
{
    ...
    for (std::size_t i = 0; i < positions_in.size(); ++i)
    {
        ...
        const bool newlineflag = addition && !previous_addition && (character == "\n") && (character ==
            previous_character) && (position == previous_position);
        if (newlineflag)
        {
