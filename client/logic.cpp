...
void client_logic_enable_client(const bool enable)
{
...
std::string client_logic_url(const std::string& address, const int port, const std::string& path)
{
...
std::string client_logic_connection_setup(std::string user, std::string hash)
{
    ...
    const std::string url = client_logic_url(address, port, sync_setup_url()) + "?user=" + encoded_user + "&pass=" + hash;

    std::string error{};
    std::string response = filter_url_http_get(url, error, true);
    const int i_response = filter::string::convert_to_int(response);

    if (i_response >= roles::guest and i_response <= roles::admin)
    {
    ...
    {
        database::logs::logv1(error, roles::translator);
        ...
        const std::string upgrade_required = filter_url_http_response_code_text(426);
        const size_t pos = error.find(upgrade_required);
...
std::string client_logic_create_note_encode(
    const std::string& bible, const int book, const int chapter, const int verse,
