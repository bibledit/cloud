...
std::string email_index(Webserver_Request& webserver_request)
{
    ...
    auto header = Assets_Header(translate("Mail"), webserver_request);
    page = header.run();

    Assets_View view;

    // Site name and email.
    if (not webserver_request.post_get("email").empty())
    {
        bool form_is_valid = true;
        std::string sitename = webserver_request.post_get("sitename");
        std::string sitemail = webserver_request.post_get("sitemail");
        if (not sitemail.empty())
        {
            if (not filter_url_email_is_valid(sitemail))
            {
                form_is_valid = false;
    ...
    // Email retrieval.
    if (not webserver_request.post_get("retrieve").empty())
    {
        ...
        int mail_count = email::receive_count(storage_error, true);
        if (storage_error.empty())
        {
            ...
            storage_success.append(" ");
            storage_success.append(std::to_string(mail_count));
            storage_success.append(".");
        }
    ...
    if (database::config::general::get_mail_storage_protocol() == "POP3S") view.set_variable(
        "storagepop3s", R"(selected="selected")");
    ...
    if (!webserver_request.post_get("send").empty())
    {
        ...
        const std::string result = email::send(database::config::general::get_site_mail_address(),
                                               database::config::general::get_site_mail_name(), "Test",
                                               "This is to check sending email.", true);
        if (result.empty())
        {
            send_success.append(" ");
