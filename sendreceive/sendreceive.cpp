...
void sendreceive_sendreceive ([[maybe_unused]] std::string bible)
{
  ...
  if (read_from_git) {
    database::logs::log<roles::translator> (sendreceive_tag (), sendreceive_sendreceive_sendreceive_text (), bible);
  } else {
    database::logs::log<roles::translator> (sendreceive_tag (), sendreceive_sendreceive_send_text (), bible);
  }
  ...
  if (success) {
    ...
    for (auto & line : lines) {
      ...
      if (passage.book()) {
        database::logs::log<roles::translator> (sendreceive_tag (), line);
        localchanges = true;
      }
    }
  }
  ...
  if (success && localchanges) {
    ...
    if (!success) {
      database::logs::log<roles::translator> (sendreceive_tag (), error);
    }
  }
  ...
  if (success) {
    ...
    if (!success || conflict || logs.size () > 1) {
      for (auto & log : logs) {
        ...
        database::logs::log<roles::translator> (sendreceive_tag (), "receive:", log);
      }
    }
    if (conflict) {
      database::logs::log<roles::translator> (sendreceive_tag (), translate ("Bibledit will resolve the conflicts"));
      filter_git_resolve_conflicts (directory, paths_resolved_conflicts, error);
      if (!error.empty ()) database::logs::log<roles::translator> (error);
      std::vector <std::string> messages;
      std::string tmp_error;
      std::string no_user {};
      filter_git_commit (directory, no_user, translate ("Bibledit resolved the conflicts"), messages, tmp_error);
      for (auto & msg : messages) database::logs::log<roles::translator> (sendreceive_tag (), "conflict resolution:", msg);
  ...
  if (success) {
    ...
    if (!success || messages.size() > 1) {
      for (auto & msg : messages) database::logs::log<roles::translator> (sendreceive_tag (), "send:", msg);
    }
  }
  ...
  if (!success) {
    ...
    database::logs::log<roles::translator> (sendreceive_tag (), msg);
  }
  {
    std::string msg;
    if (read_from_git) {
    ...
    database::logs::log<roles::translator> (sendreceive_tag (), msg, bible);
  }
#endif
}
