...
void export_web_book (std::string bible, int book, bool log)
{
  ...
  if (log) {
    database::logs::log<roles::translator> (translate("Exported to web"), ":", bible, database::books::get_english_from_id (static_cast<book_id>(book)));
  }
}
...
void export_web_index (std::string bible, bool log)
{
  ...
  if (log) database::logs::log<roles::translator> (translate("Exported to web"), ":", bible, "Index");
}
