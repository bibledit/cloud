...
HtmlText::HtmlText(const std::string& title)
{
    ...
    const pugi::xml_node title_node = head_node.append_child("title");
...
void HtmlText::open_text_style(const stylesv2::Style* style,
                               const bool note, const bool embed)
{
    const auto get_marker = [style]
    {
        if (style)
            return style->marker;
        return std::string();
...
void HtmlText::close_text_style(const bool note, const bool embed)
{
    if (note)
    {
        if (not current_note_text_style.empty())
...
void HtmlText::add_note_text(const std::string& text)
{
    ...
    if (popup_node)
    {
        const pugi::xml_node span_node2 = popup_node.append_child("span");
                        ...
                        const bool add_popup)
{
    ...
    pugi::xml_node pc_data = a_node.append_child(pugi::node_pcdata);
    pc_data.set_value(text.c_str());
...
std::string HtmlText::get_html()
{
    ...
    // Or remove the empty note container.
    if (note_count)
...
{
    const pugi::xml_node table_row_element = table_element.append_child("tr");
...
{
    const pugi::xml_node text_h_dom_element = body_node.append_child(style.c_str());
