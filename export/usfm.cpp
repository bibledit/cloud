...
void export_usfm (std::string bible, bool log)
{
  ...
  for (auto book : books) {
    ...
    if (log) database::logs::log (translate("Exported to USFM") + ": " + bible + " " + database::books::get_english_from_id (static_cast<book_id>(book)), roles::translator);
  }
  ...
  if (log) database::logs::log (translate("Exported to USFM") + ": " + bible + " " + translate("All books"), roles::translator);
}
