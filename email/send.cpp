...
void send ()
{
  ...
  for (auto id : mails) {
    ...
    if (!filter_url_email_is_valid (email)) {
      ...
      database::logs::logv1 ("Email to " + email + " was deleted because of an invalid email address");
      continue;
    }
    ...
    if (config_globals_has_crashed_while_mailing) {
      database_mail.erase (id);
      database::logs::logv1 ("Email to " + email + " with subject \"" + subject + "\" was deleted because it has caused a crash");
    ...
    if (result.empty ()) {
...
#endif
      database::logs::logv1 (result, roles::manager);
    } else {
      ...
      if (login_denied) {
        ...
        database::logs::logv1 ("Postponing sending " + std::to_string (ids.size()) + " emails", roles::manager);
        break;
      } else {
                  ...
                  [[maybe_unused]] bool verbose)
{
  ...
  if (!error.empty ()) {
    database::logs::logv1 ("Failure sending email: " + error, roles::guest);
  }
