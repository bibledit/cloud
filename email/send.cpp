...
static std::mutex sending_mutex{};
...
void send()
{
    ...
    for (auto id : mails)
    {
        ...
        {
            database_mail.erase(id);
            database::logs::log("Email to", email, "with subject \"", subject,
                                "\" was deleted because it has caused a crash");
        ...
        if (result.empty())
        {
            ...
            if constexpr (config::logic::platform() != config::logic::Platform::cloud)
                ss << "queued for sending through the Cloud";
            database::logs::log<roles::manager>(std::move(ss).str());
        }
        else
        {
            // Special handling of cases that the smart SNMP host denied login.
...
static size_t payload_source(void* ptr, size_t size, size_t nmemb, void* userp)
{
    ...
    const char* data = payload_text[upload_ctx->lines_read].c_str();
...
//template <config::logic::Platform platform>
                 ...
                 [[maybe_unused]] bool verbose)
{
    ...
    payload_text.clear();
    std::string payload;
    payload = "Date: " + std::to_string(filter::date::get_year_ad(seconds)) + "/" +
        std::to_string(filter::date::get_month_within_year(seconds)) + "/" +
        std::to_string(filter::date::get_day_within_month(seconds)) + " " +
        std::to_string(filter::date::get_hour_within_day(seconds)) + ":" + std::to_string(
            filter::date::get_minute_within_hour(seconds)) + "\n";
    ...
    payload_text.push_back(std::move(payload));
    payload_text.emplace_back("Mime-version: 1.0\n");
    payload_text.emplace_back(R"(Content-Type: multipart/alternative; boundary="------------010001060501040600060905")");
    // Empty line to divide headers from body, see RFC5322.
    payload_text.emplace_back("\n");
    // Plain text part.
    payload_text.emplace_back("--------------010001060501040600060905\n");
    payload_text.emplace_back("Content-Type: text/plain; charset=utf-8\n");
    payload_text.emplace_back("Content-Transfer-Encoding: 7bit\n");
    payload_text.emplace_back("\n");
    payload_text.emplace_back("Plain text message.\n");
    payload_text.emplace_back("--------------010001060501040600060905\n");
    payload_text.emplace_back("Content-Type: text/html; charset=\"utf-8\"\n");
    payload_text.emplace_back("Content-Transfer-Encoding: 8bit\n");
    payload_text.emplace_back("\n");
    payload_text.emplace_back("<!DOCTYPE html>\n");
    payload_text.emplace_back("<html>\n");
    payload_text.emplace_back("<head>\n");
    payload_text.emplace_back("<meta http-equiv=\"content-type\" content=\"text/html; charset=UTF-8\"></meta>\n");
    payload_text.emplace_back("<meta charset=\"utf-8\" />\n");
    payload_text.emplace_back("</head>\n");
    payload_text.emplace_back("<body>\n");
    std::vector<std::string> bodylines = filter::string::explode(body, '\n');
    for (auto& line : bodylines)
    {
        if (filter::string::trim(line).empty()) payload_text.emplace_back(" ");
...
std::string setup_information(const bool require_send, const bool require_receive)
{
    if constexpr (config::logic::platform() != config::logic::Platform::cloud)
    {
        (void)require_send;
