...
std::string user_notifications (Webserver_Request& webserver_request)
{
  ...
  view.set_variable ("editednotessubscription", filter::string::get_checkbox_status (database_config_user.get_subscribe_to_consultation_notes_edited_by_me ()));

  if (checkbox == "anynotessubscription") {
    database_config_user.set_notify_me_of_any_consultation_notes_edits (checked);
    return std::string();
  }
  view.set_variable ("anynotessubscription", filter::string::get_checkbox_status (database_config_user.get_notify_me_of_any_consultation_notes_edits ()));

  if (checkbox == "emailconfirmationyourposts") {
    database_config_user.set_notify_me_of_my_posts (checked);
    return std::string();
  }
  view.set_variable ("emailconfirmationyourposts", filter::string::get_checkbox_status (database_config_user.get_notify_me_of_my_posts ()));

  if (checkbox == "subscribednotenotification") {
    database_config_user.set_subscribed_consultation_note_notification (checked);
    return std::string();
  }
  view.set_variable ("subscribednotenotification", filter::string::get_checkbox_status (database_config_user.get_subscribed_consultation_note_notification ()));

  if (checkbox == "notesassignment") {
    database_config_user.set_assigned_to_consultation_notes_changes (checked);
    return std::string();
  }
  view.set_variable ("notesassignment", filter::string::get_checkbox_status (database_config_user.get_assigned_to_consultation_notes_changes ()));
  
  if (checkbox == "assignednotenotification") {
    database_config_user.set_assigned_consultation_note_notification (checked);
    return std::string();
  }
  view.set_variable ("assignednotenotification", filter::string::get_checkbox_status (database_config_user.get_assigned_consultation_note_notification ()));
  
  if (checkbox == "suppressemailsfromnotesyouupdated") {
    database_config_user.set_suppress_mail_from_your_updates_notes (checked);
    return std::string();
  }
  view.set_variable ("suppressemailsfromnotesyouupdated", filter::string::get_checkbox_status (database_config_user.get_suppress_mail_from_your_updates_notes ()));

  if (checkbox == "assignednotesnotification") {
    database_config_user.set_assigned_notes_statistics_notification (checked);
    return std::string();
  }
  view.set_variable ("assignednotesnotification", filter::string::get_checkbox_status (database_config_user.get_assigned_notes_statistics_notification ()));

  if (checkbox == "subscribednotesnotification") {
    database_config_user.set_subscribed_notes_statistics_notification (checked);
    return std::string();
  }
  view.set_variable ("subscribednotesnotification", filter::string::get_checkbox_status (database_config_user.get_subscribed_notes_statistics_notification ()));

  if (checkbox == "deletednotenotification") {
    database_config_user.set_deleted_consultation_note_notification (checked);
    return std::string();
  }
  view.set_variable ("deletednotenotification", filter::string::get_checkbox_status (database_config_user.get_deleted_consultation_note_notification ()));

  if (checkbox == "postponenewnotesmails") {
    database_config_user.set_postpone_new_notes_mails (checked);
    return std::string();
  }
  view.set_variable ("postponenewnotesmails", filter::string::get_checkbox_status (database_config_user.get_postpone_new_notes_mails ()));
  ...
  for (const auto& assignee : all_assignees) {
    if (checkbox == "autoassign" + assignee) {
      if (checked) {
        ...
        current_assignees = filter::string::array_diff (current_assignees, {assignee});
      }
  ...
  view.set_variable ("anyonechangesemailnotification", filter::string::get_checkbox_status (database_config_user.get_bible_changes_notification ()));

  if (checkbox == "anyonechangesonlinenotifications") {
    database_config_user.set_generate_change_notifications (checked);
    return std::string();
  }
  view.set_variable ("anyonechangesonlinenotifications", filter::string::get_checkbox_status (database_config_user.get_generate_change_notifications ()));

  if (checkbox == "pendingchangenotifications") {
    database_config_user.set_pending_changes_notification (checked);
    return std::string();
  }
  view.set_variable ("pendingchangenotifications", filter::string::get_checkbox_status (database_config_user.get_pending_changes_notification ()));
  
  if (checkbox == "mychangesemailnotifications") {
    database_config_user.set_user_changes_notification (checked);
    return std::string();
  }
  view.set_variable ("mychangesemailnotifications", filter::string::get_checkbox_status (database_config_user.get_user_changes_notification ()));
  
  if (checkbox == "mychangesonlinenotifications") {
    database_config_user.set_user_changes_notifications_online (checked);
    return std::string();
  }
  view.set_variable ("mychangesonlinenotifications", filter::string::get_checkbox_status (database_config_user.get_user_changes_notifications_online ()));
  ...
  view.set_variable ("contributorschangesonlinenotifications", filter::string::get_checkbox_status (database_config_user.get_contributor_changes_notifications_online ()));
    ...
    for (const auto& bible : bibles) {
      if (checkbox == "changenotificationbible" + bible) {
        ...
        if (checked) {
          ...
          currentbibles = filter::string::array_diff (currentbibles, {bible});
        }
  ...
  view.set_variable ("biblechecksnotification", filter::string::get_checkbox_status (database_config_user.get_bible_checks_notification ()));
