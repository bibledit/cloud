...
void export_text_usfm_book (string bible, int book, bool log)
{
  ...
  string basicUsfm = "\\id " + database::books::get_usfm_from_id_v2 (static_cast<book_id>(book)) + "\n";
  ...
  if (log) Database_Logs::log (translate("Exported to basic USFM and text") + ": " + bible + " " + database::books::get_english_from_id_v2 (static_cast<book_id>(book)), Filter_Roles::translator ());
}
