...
std::string menu_logic_main_categories(Webserver_Request& webserver_request, std::string& tooltip)
{
    ...
    std::string color = filter::css::theme_picker(current_theme_index, 1);
...
std::string menu_logic_workspace_category(Webserver_Request& webserver_request, std::string* tooltip)
{
    ...
    if (workspace_organize_acl(webserver_request))
    {
        ...
        {
            std::string item = menu_logic_create_item(workspace_index_url() + "?bench=" + std::to_string(i),
                                                      workspaces[i], true, "", "");
...
std::string menu_logic_settings_category(Webserver_Request& webserver_request, std::string* tooltip)
{
    ...
    for (auto& label : labels)
    {
        ...
        if (label == workspaces)
        {
            if (workspace_organize_acl(webserver_request))
            {
                html.push_back(menu_logic_create_item(workspace_organize_url(), menu_logic_workspace_organize_text(),
                                                      true, "", ""));
        ...
        if (label == resources)
        {
            ...
            {
                html.push_back(menu_logic_create_item(menu_logic_settings_resources_menu(), menu_logic_resources_text(),
                                                      false, "", ""));
        ...
        if (label == versifications)
        {
            ...
            {
                html.push_back(menu_logic_create_item(versification_index_url(), menu_logic_versification_index_text(),
                                                      true, "", ""));
        ...
        if (label == basic_mode)
        {
            if (webserver_request.session_logic()->get_level() > roles::guest)
            {
                html.push_back(menu_logic_create_item(
                    index_index_url() + filter::string::convert_to_string("?mode=basic"), label, true, "", ""));
                tiplabels.push_back(label);
            }
        }
