...
void receive ()
{
  ...
  for (int i = 1; i <= email_count; i++) {
    ...
    if (error.empty ()) {
      ...
      database::logs::logv1 ("Processing email from " + from + " with subject " + subject);
      ...
      if (notes_logic.handleEmailComment (from, subject, body)) {
        ...
        database::logs::logv1 ("Could not allocate email from " + from + ", subject " + subject);
        database::logs::logv1 (body);
      }
      
    } else {
      database::logs::logv1 ("Error retrieving mail: " + error);
    }
