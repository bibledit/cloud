...
void changes_modifications ()
{
  ...
  if (!lock.try_lock_for(std::chrono::milliseconds(200))) {
    database::logs::logv1 ("Change notifications: Skipping just now because another process is already generating them", roles::translator);
    return;
  }
  

  database::logs::logv1 ("Change notifications: Generating", roles::translator);
    ...
    for (const auto& bible : bibles) {
      ...
      for (auto book : books) {
        ...
        for (auto chapter : chapters) {
          ...
          database::logs::logv1 ("Change notifications: User " + user + " - Bible " + bible + " " + filter_passage_display (book, chapter, ""), roles::translator);
  ...
  for (const auto & bible : bibles) {
    ...
    for (auto book : books) {
      ...
      for (auto chapter : chapters) {
        database::logs::logv1 ("Change notifications: " + bible + " " + filter_passage_display (book, chapter, ""), roles::translator);
  ...
  database::logs::logv1 ("Change notifications: Indexing", roles::translator);
    ...
    if (days > 31) {
      ...
      for (const auto& revision : revisions) {
        ...
        if (days2 > 31) {
          ...
          database::logs::logv1 ("Removing expired downloadable revision notification: " + bible + " " + revision, roles::translator);
        }
      }
    }
  }
  ...
  database::logs::logv1 ("Change notifications: Ready", roles::translator);
}
