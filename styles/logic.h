...
std::string type_enum_to_value(Type type, bool describe = false);
...
std::string textalignment_enum_to_value(TextAlignment alignment);
...
bool has_property(const Style* style, Property property);

template <typename T>
T get_parameter(const Style* style, const Property property)
{
    if (style)
    {
        if (const auto iter = style->properties.find(property); iter != style->properties.cend())
            if (const Parameter& parameter = iter->second; std::holds_alternative<T>(parameter))
...
constexpr auto notes_numbering_restart_never{"never"}; // Never restart the notes numbering.
constexpr auto notes_numbering_restart_book{"book"}; // Restart notes numbering every book.
constexpr auto notes_numbering_restart_chapter{"chapter"}; // Restart notes numbering every chapter.
constexpr auto notes_dump_book{"book"}; // Dump the endnotes at the end of each book.
constexpr auto notes_dump_end{"end"}; // Dump the endnotes at the very end of everything.
