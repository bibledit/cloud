    ...
    for (const auto& bit1 : bits1)
    {
        ...
        if (bit1.length() == 3)
        {
            ...
            for (const auto& bit2 : bits2)
            {
                ...
                if (bit2.length() == 3)
                {
                    const std::vector<std::string> bits3 = filter_url_scandir(filter_url_create_path({
                        main_folder, bit1, bit2
                    }));
...
void Database_Notes::update_database(int identifier)
{
    ...
    update_database_internal(identifier, modified, assigned, subscriptions, bible, passage, status, severity, summary,
                             contents);
}


void Database_Notes::update_database_internal(int identifier, int modified, std::string assigned,
                                              std::string subscriptions, std::string bible, std::string passage,
                                              std::string status, int severity, std::string summary,
                                              std::string contents)
{
    ...
    sql.add(
        "SELECT modified, assigned, subscriptions, bible, passage, status, severity, summary, contents FROM notes WHERE identifier =");
    sql.add(identifier);
    sql.add(";");
    ...
    sql.clear();
    sql.add(
        "INSERT INTO notes (identifier, modified, assigned, subscriptions, bible, passage, status, severity, summary, contents) VALUES (");
    sql.add(identifier);
    sql.add(",");
    sql.add(modified);
    sql.add(",");
...
std::vector<int> Database_Notes::select_notes(const Selector& selector)
{
    ...
    case EditSelector::since_yesterday:
        ...
        time = filter::date::get_seconds_since_epoch() - 1 * 24 * 3600 - filter::date::get_hour_within_day(
            filter::date::get_seconds_since_epoch()) * 3600;
        break;
    case EditSelector::today:
        // Select notes that have been edited today.
        time = filter::date::get_seconds_since_epoch() - filter::date::get_hour_within_day(
            filter::date::get_seconds_since_epoch()) * 3600;
        break;
    }
    if (time != 0)
...
std::string Database_Notes::get_severity(int identifier)
{
    ...
    if ((severity >= 0) && (severity < static_cast<int>(standard.size()))) severitystring = standard[static_cast<size_t>
        (severity)];
...
std::vector<int> Database_Notes::get_notes_in_range_for_bibles(int lowId, int highId, std::vector<std::string> bibles,
                                                               bool anybible)
{
    std::vector<int> identifiers;
