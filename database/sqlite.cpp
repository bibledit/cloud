...
static int callback(void* userdata, const int argc, char** argv, char** column_names)
{
    auto* sqlite_reader = static_cast<Reader*>(userdata);
    for (int i = 0; i < argc; i++)
    {
        // Handle nullptr field.
        if (argv[i] == nullptr)
            sqlite_reader->result[column_names[i]].emplace_back("");
        else
            sqlite_reader->result[column_names[i]].emplace_back(argv[i]);
    }
    return 0;
}
...
sqlite3* connect_file(const std::string& filename)
{
    ...
    if (const int rc = sqlite3_open(filename.c_str(), &db); rc)
    {
        ...
        sqlite::error(db, "Database " + filename, error);
        return nullptr;
    }
...
std::map<std::string, std::vector<std::string>> query(sqlite3* db, const std::string& sql)
{
    ...
    {
        execute_mutex.lock();
        const int rc = sqlite3_exec(db, sql.c_str(), callback, &reader, &error);
        execute_mutex.unlock();
        if (rc != SQLITE_OK)
            sqlite::error(db, sql, error);
    }
...
{
    const std::string file = get_file(database);
    bool ok = false;
    ...
    if (filter_url_filesize(file) > 0)
    {
        ...
        const auto query = "PRAGMA integrity_check;";
        std::map<std::string, std::vector<std::string>> result = sqlite::query(db, query);
        if (const std::vector<std::string> health = result["integrity_check"];
            health.size() == 1)
        {
            if (health.at(0) == "ok")
                ok = true;
...
void error(sqlite3* database, const std::string& prefix, const char* error)
{
    ...
    if (database)
    {
        ...
        if (const char* filename = sqlite3_db_filename(database, "main"); filename)
        {
            message.append(" - ");
...
void SqliteDatabase::add(const int value)
{
    m_sql.append(" ");
