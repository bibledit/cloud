  ...
  if (!webserver_request.post_get("submit").empty ()) {
    ...
    if (form_is_valid) {
      ...
      if (webserver_request.session_logic()->attempt_login (name, name, touch_enabled)) {
        // Log the login.
        database::logs::logv1 ("User " + webserver_request.session_logic ()->get_username () + " logged in");
      } else {
        ...
        database::logs::logv1 ("Public account created for user " + webserver_request.session_logic ()->get_username () + " with email " + email);
      }
    }
  }
