...
void Editor_Html2Usfm::process_node(pugi::xml_node& node)
{
  switch (node.type ()) {
    ...
    default:
    {
      database::logs::logv1 ("XML node " + std::string(node.name ()) + " not handled while saving editor text");
      break;
    }
  }
}
...
void Editor_Html2Usfm::process_note_citation (pugi::xml_node& node)
{
  ...
  if (note_p_element) {
    ...
    database::logs::logv1 ("Discarding note with id " + id);
  }
}
...
void Editor_Html2Usfm::post_process ()
{
  ...
  for (const auto& element : m_word_level_attributes) {
    database::logs::logv1 ("Discarding unprocessed word-level attribute with key: " + element.first + " and value: " + element.second);
  }
  for (const auto& element : m_milestone_attributes) {
    database::logs::logv1 ("Discarding unprocessed milestone attribute with key: " + element.first + " and value: " + element.second);
  }
}
