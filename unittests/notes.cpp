...
void test_database_notes ()
{
  ...
  // Test operations on the checksum.
  {
    ...
    checksum = database_notes.get_checksum_v12 (oldidentifier);
    evaluate (__LINE__, __func__, "", checksum);
    database_notes.set_subscribers_v1 (oldidentifier, {"subscribers"});
    checksum = database_notes.get_checksum_v12 (oldidentifier);
    evaluate (__LINE__, __func__, false, checksum.empty());
    // New.
    ...
    checksum = database_notes.get_checksum_v12 (oldidentifier);
    evaluate (__LINE__, __func__, "", checksum);
    database_notes.set_summary_v1 (oldidentifier, "new");
    checksum = database_notes.get_checksum_v12 (oldidentifier);
    evaluate (__LINE__, __func__, false, checksum.empty());
    // New
    database_notes.set_checksum_v12 (newidentifier, "");
    ...
    checksum = database_notes.get_checksum_v12 (oldidentifier);
    evaluate (__LINE__, __func__, "", checksum);
    database_notes.set_contents_v1 (oldidentifier, "new");
    checksum = database_notes.get_checksum_v12 (oldidentifier);
    evaluate (__LINE__, __func__, false, checksum.empty());
    // New.
    database_notes.delete_checksum_v12 (newidentifier);
  ...
  // Testing public notes.
  {
    ...
    evaluate (__LINE__, __func__, false, database_notes.get_public_v1 (oldidentifier2));
    evaluate (__LINE__, __func__, false, database_notes.get_public_v1 (oldidentifier3));
    evaluate (__LINE__, __func__, false, database_notes.get_public_v1 (oldidentifier1 + 1));
  ...
  // Bulk notes transfer elaborate tests for version 1.
  {
    ...
    database_notes.set_bulk_v1 (json);
  ...
  // Test converting a note from the old format to the new JSON format.
  {
    refresh_sandbox (true);
    Database_State::create ();
    Database_Login::create ();
  ...
  // Test automatic and gradual upgrading notes from v1 to v2. Todo
  {
    ...
    // Copy the consultation notes of v1 into place.
    string consultations_directory = database_notes.main_folder_v12 ();
    string testing_data_dir = filter_url_create_root_path ("unittests", "tests", "consultationsv1");
    filter_url_dir_cp (testing_data_dir, consultations_directory);

    // Index the notes in the old v1 format.
    database_notes.sync_v12 ();
    
    // Check the current note identifiers.
    vector <int> standard_identifiers = { 101128049, 222128049, 333128049, 444214739, 444244221 };
    vector <int> identifiers = database_notes.get_identifiers_v12 ();
    evaluate (__LINE__, __func__, standard_identifiers, identifiers);

    // Check the current note checksums.
    string standard_checksum = "03b290ddaedfab5649ae3bf444026bb1";
    string checksum = database_notes.get_multiple_checksum_v12 (identifiers);
    evaluate (__LINE__, __func__, standard_checksum, checksum);

    // Convert some of them.
    vector <int> journal;
    bool all_done = database_notes.gradually_convert_v1_to_v2 (3, journal);
    evaluate (__LINE__, __func__, false, all_done);

    // Index them mixed including new.
    database_notes.sync_v12 ();

    // Compare checksum old and new.
    checksum = database_notes.get_multiple_checksum_v12 (identifiers);
    evaluate (__LINE__, __func__, standard_checksum, checksum);
    
    // Check whether a known selection is in the new format, and the rest in the old format.
    for (unsigned int i = 0; i < standard_identifiers.size(); i++) {
      evaluate (__LINE__, __func__, i >= 3, database_notes.is_v1 (standard_identifiers[i]));
    }

    // Check the current note identifiers.
    identifiers = database_notes.get_identifiers_v12 ();
    evaluate (__LINE__, __func__, standard_identifiers, identifiers);

    // Convert the remainder of the notes.
    all_done = database_notes.gradually_convert_v1_to_v2 (3, journal);
    evaluate (__LINE__, __func__, true, all_done);
    evaluate (__LINE__, __func__, standard_identifiers, journal);

    // Check that all notes are in the new format.
    for (unsigned int i = 0; i < standard_identifiers.size(); i++) {
      evaluate (__LINE__, __func__, false, database_notes.is_v1 (standard_identifiers[i]));
    }

    // Compare checksum of all new notes.
    checksum = database_notes.get_multiple_checksum_v12 (identifiers);
    evaluate (__LINE__, __func__, standard_checksum, checksum);
  }
}
