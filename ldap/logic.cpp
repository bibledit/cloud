...
void ldap_logic_initialize ()
{
  ...
  if (file_or_dir_exists (path)) {
    ...
    if (ldap_logic_is_on (true)) {
      database::logs::logv1 ("Using LDAP for authentication");
    }
  }
}
...
bool ldap_logic_is_on (bool log)
{
  if (ldap_logic_uri.empty ()) {
    if (log) database::logs::logv1 ("LDAP server configuration lacks the URI");
    return false;
  }
  if (ldap_logic_binddn.empty ()) {
    if (log) database::logs::logv1 ("LDAP server configuration lacks the bind dn");
    return false;
  }
  if (ldap_logic_basedn.empty ()) {
    if (log) database::logs::logv1 ("LDAP server configuration lacks the base dn");
    return false;
  }
  if (ldap_logic_scope.empty ()) {
    if (log) database::logs::logv1 ("LDAP server configuration lacks the scope");
    return false;
  }
  if (ldap_logic_filter.empty ()) {
    if (log) database::logs::logv1 ("LDAP server configuration lacks the search filter");
    return false;
  }
  if (ldap_logic_role.empty ()) {
    if (log) database::logs::logv1 ("LDAP server configuration lacks the role field");
    return false;
  }
  return true;
}
...
bool ldap_logic_fetch (const std::string& user, const std::string& password, bool& access, std::string& email, int& role, bool log)
{
  ...
  if (log) {
    ...
    database::logs::logv1 ("LDAP query\n" + command + "\n" + output, roles::admin);
  }
