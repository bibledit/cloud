...
void sendreceive_sendreceive ([[maybe_unused]] std::string bible)
{
  ...
  // Check on Bible.
  if (bible.empty ()) {
    database::logs::logv1 (sendreceive_tag () + "No Bible to send and receive", roles::translator);
    return;
  }
  ...
  if (!file_or_dir_exists (directory)) {
    ...
    database::logs::logv1 (sendreceive_tag () + msg);
    return;
  }
  ...
  if (read_from_git) {
    database::logs::logv1 (sendreceive_tag () + sendreceive_sendreceive_sendreceive_text () + bible, roles::translator);
  } else {
    database::logs::logv1 (sendreceive_tag () + sendreceive_sendreceive_send_text () + bible, roles::translator);
  }
  ...
  if (success) {
    ...
    for (auto & line : lines) {
      ...
      if (passage.book()) {
        database::logs::logv1 (sendreceive_tag () + line, roles::translator);
        localchanges = true;
      }
    }
  }
  ...
  if (success && localchanges) {
    ...
    if (!success) {
      database::logs::logv1 (sendreceive_tag () + error, roles::translator);
    }
  }
  ...
  if (success) {
    ...
    if (!success || conflict || logs.size () > 1) {
      for (auto & log : logs) {
        ...
        database::logs::logv1 (sendreceive_tag () + "receive: " + log, roles::translator);
      }
    }
    if (conflict) {
      database::logs::logv1 (sendreceive_tag () + translate ("Bibledit will resolve the conflicts"), roles::translator);
      filter_git_resolve_conflicts (directory, paths_resolved_conflicts, error);
      if (!error.empty ()) database::logs::logv1 (error, roles::translator);
      std::vector <std::string> messages;
      std::string tmp_error;
      std::string no_user {};
      filter_git_commit (directory, no_user, translate ("Bibledit resolved the conflicts"), messages, tmp_error);
      for (auto & msg : messages) database::logs::logv1 (sendreceive_tag () + "conflict resolution: " + msg, roles::translator);
  ...
  if (success) {
    ...
    if (!success || messages.size() > 1) {
      for (auto & msg : messages) database::logs::logv1 (sendreceive_tag () + "send: " + msg, roles::translator);
    }
  }
  ...
  if (!success) {
    ...
    database::logs::logv1 (sendreceive_tag () + msg, roles::translator);
  }
  {
    std::string msg;
    if (read_from_git) {
    ...
    database::logs::logv1 (sendreceive_tag () + msg + " " + bible, roles::translator);
  }
#endif
}
