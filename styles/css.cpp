...
{
    m_code.emplace_back(".superscript, [class^='i-note'] { font-size: x-small; vertical-align: super; }");
    if (exports_enabled)
    {
        add_exports_styles();
    }
    ...
    std::ranges::for_each(database::styles::get_markers(m_stylesheet), [this](const auto& marker)
    {
...
// Evaluates the style to decide how it should look.
...
void Styles_Css::add(const stylesv2::Style* style, const bool paragraph, const bool keep_with_next)
{
    ...
    const std::string class_name{style->marker};
    ...
    if (paragraph and style->paragraph)
    {
        const float points{static_cast<float>(style->paragraph.value().font_size)};
        const float percents{points * 100 / 12};
        if (const int fontsize = filter::string::convert_to_int(percents); fontsize != 100)
        {
    ...
    if (paragraph and style->paragraph)
    {
        ...
        if (sp.italic != TwoState::off)
            m_code.emplace_back("font-style: italic;");
        if (sp.bold != TwoState::off)
            m_code.emplace_back("font-weight: bold;");
        if (sp.underline != TwoState::off)
            m_code.emplace_back("text-decoration: underline;");
        if (sp.smallcaps != TwoState::off)
            m_code.emplace_back("font-variant: small-caps;");
    }

    // For inline text.
    // Italics, bold, underline, small caps can be Off or On or Inherit or Toggle.
    ...
    if (!paragraph)
    {
        if (style->character)
        {
            if (const FourState state = style->character.value().italic;
                state == FourState::on or state == FourState::toggle)
            {
                m_code.emplace_back("font-style: italic;");
            }
    ...
    if (paragraph and style->paragraph)
    {
        ...
        if (keep_with_next)
        {
            m_code.emplace_back("page-break-inside: avoid;");
        }
    }
    ...
    if (not paragraph)
    {
        if (style->character)
        {
            ...
            {
                m_code.emplace_back("font-size: x-small;");
                m_code.emplace_back("vertical-align: super;");
            }
...
std::string Styles_Css::css(const std::string& path) const
{
    const std::string css = filter::string::implode(m_code, "\n");
    if (not path.empty())
    {
...
{
    m_code.emplace_back("body { }");
    m_code.emplace_back("p { margin-top: 0; margin-bottom: 0; }");
    m_code.emplace_back("p.page { page-break-after: always; }");
    ...
    m_code.emplace_back(".dropcaps:nth-of-type(1) { font-size: 300%; line-height: 0.85em; margin-right: 0.03em; margin-bottom:-0.25em; }");
    m_code.emplace_back("a { text-decoration: none; background: none; }");
    m_code.emplace_back("a:visited { color: #5a3696; }");
    m_code.emplace_back("a:active { color: #faa700; }");
    m_code.emplace_back("a:hover { text-decoration: underline; }");
    m_code.emplace_back(".breadcrumbs { font-size: normal; }");
    m_code.emplace_back(".navigationbar { font-size: normal; }");
    m_code.emplace_back(".popup { position:absolute; display:none; background-color:lightyellow; border:.1em solid; width:15em; height:auto; padding:1em; font-size:1.5em; text-indent:0em; margin 0.5em 0.5em 0.5em 0.5em; }");
    m_code.emplace_back("a:hover .popup { display:block; }");
}
