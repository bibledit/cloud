...
std::string resource_organize(Webserver_Request& webserver_request)
{
    ...
    if (!add.empty())
    {
        if (add == resource_logic_rich_divider())
        {
            ...
            if (is_def)
                redirect_browser(webserver_request,
                                 filter_url_build_http_query(resource_divider_url(), {{"type", "def"}}));
            else
        ...
        else
        {
            ...
            if (!is_def)
                webserver_request.database_config_user()->add_updated_setting(
                    Sync_Logic::settings_send_resources_organization);
            else
    ...
    {
        Dialog_Entry dialog_entry = Dialog_Entry("organize", translate("Please enter the number of verses"),
                                                 std::to_string(
                                                     webserver_request.database_config_user()->
                                                                       get_resource_verses_before()), "before",
                                                 translate(
                                                     "How many verses of context to display before the focused verse."));
    ...
    if (webserver_request.query.count("after"))
    {
        Dialog_Entry dialog_entry = Dialog_Entry("organize", translate("Please enter the number of verses"),
                                                 std::to_string(
                                                     webserver_request.database_config_user()->
                                                                       get_resource_verses_after()), "after",
                                                 translate(
                                                     "How many verses of context to display after the focused verse."));
        page += dialog_entry.run();
        return page;
    }
    ...
    view.set_variable(
        "related", filter::string::get_checkbox_status(
            webserver_request.database_config_user()->get_include_related_passages()));
    ...
    if (webserver_request.query.count("adddefaultresources"))
    {
        ...
        joined_resources.insert(joined_resources.end(), default_active_resources.begin(),
                                default_active_resources.end());
    ...
    if (webserver_request.query.count("install"))
    {
        ...
        view.set_variable(
            "success",
            translate("Will install the resources.") + " " + translate("The journal displays the progress."));
    }
