...
std::string notes_actions(Webserver_Request& webserver_request)
{
    ...
    if (webserver_request.query.count("markdel"))
    {
        ...
        success = translate("The note will be deleted after a week.") + " " + translate(
            "Adding a comment to the note cancels the deletion.");
    }
    ...
    for (auto& assignee : assignees)
    {
        ...
        {
            assigneeblock << "<a href=" << std::quoted("?id=" + std::to_string(id) + "&unassign=" + assignee) << "> ["
                << translate("unassign") << "]</a>";
            assigneeblock << " | ";
        }
    }
