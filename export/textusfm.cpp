...
void export_text_usfm_book (string bible, int book, bool log)
{
  ...
  if (!file_or_dir_exists/*_cpp17*/ (textDirectory)) filter_url_mkdir (textDirectory);
  ...
  // Basic USFM.
  if (file_or_dir_exists/*_cpp17*/ (usfmFilename)) filter_url_unlink_cpp17 (usfmFilename);
