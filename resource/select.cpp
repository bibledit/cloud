...
std::string resource_select(Webserver_Request& webserver_request)
{
    ...
    // This selection mechanism here shows that title only.
    {
        ...
        if constexpr (config::logic::platform() == config::logic::Platform::cloud)
            raw_resources = database::config::general::get_comparative_resources();
        if constexpr (config::logic::platform() != config::logic::Platform::cloud)
            raw_resources = resource_logic_comparative_resources_get_list_on_client();
    ...
    // This selection mechanism here shows that title only.
    {
        ...
        std::vector<std::string> raw_resources;
        if constexpr (config::logic::platform() == config::logic::Platform::cloud)
            raw_resources = database::config::general::get_translated_resources();
        if constexpr (config::logic::platform() != config::logic::Platform::cloud)
            raw_resources = resource_logic_translated_resources_get_list_on_client();
        for (const auto& raw_resource : raw_resources)
        {
            std::string title{};
