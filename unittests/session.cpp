...
TEST (session, logic1)
{
  ...
  EXPECT_EQ (true, request.session_logic ()->loggedIn ());
  EXPECT_EQ (session_admin_credentials (), request.session_logic ()->currentUser ());
  EXPECT_EQ (Filter_Roles::admin (), request.session_logic ()->currentLevel ());
  ...
  EXPECT_EQ (username, request.session_logic ()->currentUser ());
...
TEST (session, logic2)
{
  ...
  Webserver_Request request;
  EXPECT_TRUE (request.session_logic ()->loggedIn ());
  EXPECT_EQ (session_admin_credentials (), request.session_logic ()->currentUser ());
  EXPECT_EQ (Filter_Roles::admin (), request.session_logic ()->currentLevel ());
...
TEST (session, logic3)
{
  ...
  EXPECT_TRUE (request.session_logic ()->loggedIn ());
  EXPECT_EQ (username, request.session_logic ()->currentUser ());
  EXPECT_EQ (level, request.session_logic ()->currentLevel ());
  config_globals_client_prepared = false;
}
...
TEST (session, logic4)
{
  ...
  EXPECT_TRUE (request1.session_logic ()->loggedIn ());
  
  // Check whether logged in also from another session.
  Webserver_Request request2;
  request2.session_identifier = session;
  EXPECT_TRUE (request2.session_logic ()->loggedIn ());
  EXPECT_EQ (username, request2.session_logic ()->currentUser ());
  EXPECT_EQ (level, request2.session_logic ()->currentLevel ());
  ...
  EXPECT_FALSE (request4.session_logic ()->loggedIn ());
  EXPECT_EQ (std::string(), request4.session_logic ()->currentUser ());
  EXPECT_EQ (Filter_Roles::guest(), request4.session_logic ()->currentLevel ());
  ...
  EXPECT_TRUE (request5.session_logic ()->loggedIn ());
...
TEST (session, logic5)
{
  ...
  EXPECT_FALSE (request3.session_logic ()->loggedIn ());
  ...
  EXPECT_EQ (true, request5.session_logic ()->loggedIn ());
