...
{
  database::logs::logv1 ("Importing Bibles from " + tarball);
  ...
  if (!error.empty ()) {
    database::logs::logv1 ("Importing Bibles failure: " + error);
    return;
  }
  ...
  for (auto file : files) {
    ...
    database::logs::logv1 ("Importing from file " + file);
    std::string path = filter_url_create_path ({directory, file});
    ...
    for (auto & book_chapter_data : book_chapter_text) {
      if (book_chapter_data.m_book > 0) {
        ...
        database::logs::logv1 ("Imported " + bible + " " + bookname + " " + std::to_string (book_chapter_data.m_chapter));
      } else {
        // Import error.
        database::logs::logv1 ("Could not import this file: " + file);
      }
    }
  }
  ...
  tasks_logic_queue (tasks::enums::task::reindex_bibles, {"1"});

  // Ready, hallelujah!
  database::logs::logv1 ("Importing Bibles ready");
}
...
{
  database::logs::logv1 ("Importing Consultation Notes from " + tarball);
  ...
  std::string error= filter::archive::microtar_unpack (tarball, directory);
  if (!error.empty ()) {
    database::logs::logv1 ("Importing Consultation Notes failure: " + error);
    return;
  }
  ...
  // Ready, hallelujah!
  database::logs::logv1 ("Importing Consultation Notes ready");
}
...
{
  database::logs::logv1 ("Importing Resources from " + tarball);
  ...
  std::string error= filter::archive::microtar_unpack (tarball, directory);
  if (!error.empty ()) {
    database::logs::logv1 ("Importing Resources failure: " + error);
    return;
  }
  ...
  for (auto file : files) {
    ...
    // Get the file's contents for import.
    database::logs::logv1 ("Importing " + file);
    std::string path = filter_url_create_path ({directory, file});
    std::string data = filter_url_file_get_contents (path);
  ...
  // Ready, hallelujah!
  database::logs::logv1 ("Importing Resources ready");
}
