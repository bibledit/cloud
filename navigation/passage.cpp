...
string Navigation_Passage::get_books_fragment (void * webserver_request, string bible)
{
  ...
  for (auto book : books) {
    ...
    string bookType = database::books::book_type_to_string (database::books::get_type_v2 (book));
...
string Navigation_Passage::get_history_back (void * webserver_request)
{
  ...
  for (size_t i = 0; i < passages.size(); i++) {
    ...
    string bookType = database::books::book_type_to_string (database::books::get_type_v2 (passages[i].m_book));
