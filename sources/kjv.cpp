                             ...
                             bool & within_verse, std::string & lemma)
{
  ...
  if (element_name == "verse") {
    ...
    if (within_verse) {
      ...
      database::logs::logv1 (location + ": Failed to parse element " + element_name + " with value " + english);
    }
  }
}
...
{
  database::logs::logv1 ("Parsing data from Crosswire's KJV XML file");
  Database_Kjv database_kjv;
  database_kjv.create ();
    ...
    for (pugi::xml_node osisText : osis.children ()) {
      for (pugi::xml_node divbook : osisText.children ()) {
        if (strcmp (divbook.name (), "div") == 0) {
          book++;
          database::logs::logv1 ("Book " + std::to_string (book));
          chapter = 0;
  ...
  database::logs::logv1 ("Finished parsing data from the KJV XML file");
}
