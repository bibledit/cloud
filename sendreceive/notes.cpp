...
void sendreceive_notes ()
{
  if (sendreceive_notes_watchdog) {
    ...
    if (time < (sendreceive_notes_watchdog + 900)) {
      database::logs::log (sendreceive_notes_text () + translate("Still busy"), roles::translator);
      return;
    }
    database::logs::log (sendreceive_notes_text () + translate("Watchdog timeout"), roles::translator);
  }
  ...
  if (success) database::logs::log (sendreceive_notes_up_to_date_text (), roles::translator);
...
bool sendreceive_notes_upload ()
{
  ...
  database::logs::log (sendreceive_notes_sendreceive_text (), roles::translator);
  ...
  if (users.empty ()) {
    database::logs::log (sendreceive_notes_text () + translate("No local user found"), roles::translator);
    return false;
  }
  ...
  for (auto identifier : notes) {
    ...
    database::logs::log (sendreceive_notes_text () + translate("Sending note to server") + ": " + summary, roles::translator);
    ...
    for (auto note_action : note_actions) {
      ...
      if (!error.empty ()) {
        database::logs::log (sendreceive_notes_text () + "Failure sending note: " + error, roles::translator);
        return false;
      }
...
bool sendreceive_notes_download (int lowId, int highId)
{
  ...
  if (!healthy) {
    database::logs::log (sendreceive_notes_text () + "Abort receive just now because of database problems", roles::translator);
    return false;
  }
  ...
  if (!error.empty ()) {
    database::logs::log (sendreceive_notes_text () + "Failure requesting totals: " + error, roles::translator);
    return false;
  }
  ...
  response = sync_logic.post (post, url, error);
  if (!error.empty ()) {
    database::logs::log (sendreceive_notes_text () + "Failure requesting identifiers: " + error, roles::translator);
    return false;
  }
  ...
  for (auto identifier : identifiers) {
    ...
    database::logs::log (sendreceive_notes_text () + "Deleting because it is not on the server: " + summary, roles::translator);
  }
  ...
  if (!identifiers_bulk_download.empty ()) {
    ...
    if (identifiers_bulk_download.size () >= 3) {
      database::logs::log (sendreceive_notes_text () + "Receiving multiple notes: " + std::to_string (identifiers_bulk_download.size ()), roles::manager);
    }
    ...
    if (!error.empty ()) {
      database::logs::log (sendreceive_notes_text () + "Failure requesting multiple notes: " + error, roles::consultant);
      return false;
    }
    ...
    if (identifiers_bulk_download.size () < 3) {
      for (auto & summary : summaries) {
        database::logs::log (sendreceive_notes_text () + "Receiving: " + summary, roles::manager);
      }
    }
  }
...
{
  sendreceive_notes_watchdog = filter::date::get_seconds_since_epoch ();
}
