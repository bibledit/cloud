...
{
  database::logs::logv1 ("Refreshing list of SWORD modules");
  ...
  if (!sword_logic_installmgr_synchronize_configuration_with_master ()) {
    database::logs::logv1 ("Failed to synchronize SWORD configuration with the master remote source list");
  ...
  for (const auto& remote_source : remote_sources) {
    ...
    if (!sword_logic_installmgr_refresh_remote_source (remote_source)) {
      database::logs::logv1 ("Error refreshing remote source " + remote_source);
    }
#else
...
#endif
    database::logs::logv1 (remote_source + ": " + std::to_string (modules.size ()) + " modules");
  }
  ...
  database::logs::logv1 ("Ready refreshing SWORD module list");
}
...
void sword_logic_install_module_schedule (const std::string& source, const std::string& module)
{
  ...
  if (tasks_logic_queued (tasks::enums::task::install_sword_module, {source, module})) return;
  
  // Schedule it.
  tasks_logic_queue (tasks::enums::task::install_sword_module, {source, module});
}


void sword_logic_install_module (const std::string& source_name, const std::string& module_name)
{
  database::logs::logv1 ("Install SWORD module " + module_name + " from source " + source_name);
  std::string sword_path {sword_logic_get_path ()};
  ...
  if (source == installMgr->sources.end()) {
    database::logs::logv1 ("Could not find remote source " + source_name);
  } else {
    ...
    if (it == rmgr->Modules.end()) {
      database::logs::logv1 ("Remote source " + source_name + " does not make available module " + module_name);
    } else {
      module = it->second;
      int error = installMgr->installModule(mgr, 0, module->getName(), is);
      if (error) {
        database::logs::logv1 ("Error installing module " + module_name);
      } else {
        database::logs::logv1 ("Installed module " + module_name);
      }
    }
  }
...
{
  database::logs::logv1 ("Uninstall SWORD module " + module);
  std::string out_err;
...
std::map <int, std::string> sword_logic_get_bulk_text (const std::string& module, const int book, const int chapter, const std::vector <int>& verses)
{
  ...
  if (result != 0) database::logs::logv1 (error);
...
{
  database::logs::logv1 ("Updating installed SWORD modules");
  ...
  database::logs::logv1 ("Ready updating installed SWORD modules");
}
...
{
#ifndef HAVE_CLIENT
  database::logs::logv1 ("Trimming the installed SWORD modules");
  ...
  database::logs::logv1 ("Ready trimming the SWORD caches and modules");
#endif
}
...
#ifdef HAVE_SWORD
  sword::SWMgr *mgr = new sword::SWMgr();
  if (!mgr->config) database::logs::logv1 ("ERROR: Please configure SWORD first.");
...
bool sword_logic_installmgr_refresh_remote_source ([[maybe_unused]] const std::string& name)
{
  ...
  if (source == installMgr->sources.end()) {
    database::logs::logv1 ("Could not find remote source " + name);
  } else {
