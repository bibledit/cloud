...
void test_archive ()
{
  ...
  // Test zip entire folder.
  {
    ...
    int max = 3334;
    if (size > max) evaluate (__LINE__, __func__, "Should be no larger than " + convert_to_string (max) + " bytes", "");
  ...
  // Test tar gzip folder.
  {
    ...
    evaluate (__LINE__, __func__, true, file_or_dir_exists (tarball));
    int size = filter_url_filesize (tarball);
    if ((size < 618) || (size > 630)) evaluate (__LINE__, __func__, "between 618 and 630", convert_to_string (size));
