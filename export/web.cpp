...
void export_web_book (string bible, int book, bool log)
{
  ...
  string bibleBookText = bible + " " + database::books::get_english_from_id_v2 (static_cast<book_id>(book));
  ...
  htmlHeader.create ({
    ...
    pair (translate (database::books::get_english_from_id_v2 (static_cast<book_id>(book))), filter_url_html_file_name_bible ())
  });
  ...
  for (size_t c = 0; c < chapters.size(); c++) {
    ...
    breadcrumbs_navigator.push_back (pair (translate (database::books::get_english_from_id_v2 (static_cast<book_id>(book))), filter_url_html_file_name_bible ()));
    if (!is_first_chapter) {
    ...
    if (!feedback_email.empty ()) {
      ...
      string subject = translate ("Comment on") + " " + bible + " " + database::books::get_english_from_id_v2 (static_cast<book_id>(book)) + " " + convert_to_string (chapter);
  ...
  if (log) Database_Logs::log (translate("Exported to web") + ": " + bible + " " + database::books::get_english_from_id_v2 (static_cast<book_id>(book)), Filter_Roles::translator ());
}
...
void export_web_index (string bible, bool log)
{
  ...
  for (auto book : books) {
    ...
    html_text_rich_bible_index.add_link (html_text_rich_bible_index.current_p_node,  filter_url_html_file_name_bible ("", book), "", translate (database::books::get_english_from_id_v2 (static_cast<book_id>(book))), "", " " + translate (database::books::get_english_from_id_v2 (static_cast<book_id>(book))) + " ");
    html_text_rich_bible_index.add_text ("|");
  }
