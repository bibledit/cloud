  ...
  for (auto & identifier : identifiers) {
    ...
    if (passage_inclusion_selector) {
      ...
      for (auto & passage : include_passages) {
        std::string usfm = database::bibles::get_chapter (bible, passage.book(), passage.chapter());
        std::string text = filter::usfm::get_verse_text (usfm, filter::string::convert_to_int (passage.verse()));
