...
void checks_french::space_before_after_punctuation(const std::string& bible,
                                                   ...
                                                   const std::map<int, std::string>& texts)
{
    ...
    const std::vector<std::string> right_punctuation{std::string(right_guillemet), "!", "?", ":", ";"};
    for (const auto& verse_text : texts)
    {
        const int verse = verse_text.first;

        {
            std::string text = verse_text.second;
            size_t pos = text.find(left_guillemet);
            while (pos != std::string::npos)
            {
                text.erase(0, pos + left_guillemet.size());
                const bool space_follows = text.find(' ') == 0;
                const bool nbsp_follows = text.find(nbsp) == 0;
                const bool nnbsp_follows = text.find(nnbsp) == 0;
                if (space_follows)
                {
                    const std::string message = std::string(left_guillemet) + " - " + checks::issues::text(
                        checks::issues::issue::should_be_followed_by_a_no_break_space_rather_than_a_plain_space_in_french);
                    database::check::record_output(bible, book, chapter, verse, message);
                }
                else if (!nbsp_follows and !nnbsp_follows)
                {
                    const std::string message = std::string(left_guillemet) + " - " + checks::issues::text(
                        checks::issues::issue::should_be_followed_by_a_no_break_space_in_french);
                    database::check::record_output(bible, book, chapter, verse, message);
                }
        ...
        for (const auto& punctuation : right_punctuation)
        {
            ...
            while (pos != std::string::npos)
            {
                if (pos > 0)
                {
                    ...
                    {
                        const std::string message = punctuation + " - " + checks::issues::text(
                            checks::issues::issue::should_be_preceded_by_a_no_break_space_rather_than_a_plain_space_in_french);
                        database::check::record_output(bible, book, chapter, verse, message);
                    }
                    else if (preceding_character == nbsp)
                    {
                        /* This is OK. */
                    }
                    else if (preceding_character == nnbsp)
                    {
                        /* This is OK. */
                    }
                    else
                    {
                        const std::string message = punctuation + " - " + checks::issues::text(
                            checks::issues::issue::should_be_preceded_by_a_no_break_space_in_french);
                        database::check::record_output(bible, book, chapter, verse, message);
                    }
                }
                // Prepare for next iteration.
                text = filter::string::unicode_string_substr(text, pos + 1,
                                                             filter::string::unicode_string_length(text) - pos - 1);
                                   ...
                                   const std::vector<std::map<int, std::string>>& verses_paragraphs)
{
    ...
    for (unsigned int paragraph_counter = 0; paragraph_counter < verses_paragraphs.size(); paragraph_counter++)
    {
        ...
        if (paragraph_counter)
        {
            ...
            {
                const int verse = verses_paragraph.begin()->first;
                if (const std::string& text = verses_paragraph.begin()->second; not text.empty())
                {
                    const std::string character = filter::string::unicode_string_substr(text, 0, 1);
                    if (character != left_guillemet)
                    {
                        const std::string message = checks::issues::text(
                            checks::issues::issue::the_previous_paragraph_contains_a_citation_not_closed_with_a_right_guillemet_therefore_the_current_paragraph_is_expected_to_start_with_a_left_guillemet_to_continue_that_citation_in_french);
                        database::check::record_output(bible, book, chapter, verse, message);
                    }
                }
            }
        ...
        // Count the number of left and right guillemets in the paragraph.
        int last_verse{0};
        ...
        int opener_count{0};
        filter::string::replace(std::string(left_guillemet), "", paragraph, &opener_count);
        int closer_count{0};
        filter::string::replace(std::string(right_guillemet), "", paragraph, &closer_count);
        ...
        // Whether there's too many left guillemets.
        if (opener_count > (closer_count + 1))
        {
            const std::string message = checks::issues::text(
                checks::issues::issue::the_paragraph_contains_more_left_guillements_than_needed);
            database::check::record_output(bible, book, chapter, last_verse, message);
        }

        // Whether there's too many right guillemets.
        if (closer_count > opener_count)
        {
            const std::string message = checks::issues::text(
                checks::issues::issue::the_paragraph_contains_more_right_guillements_than_needed);
            database::check::record_output(bible, book, chapter, last_verse, message);
        }
    }
}
