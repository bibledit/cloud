...
void trash_change_notification (Webserver_Request& webserver_request, int id)
{
  ...
  database::logs::logv1 (username + " removed change notification " + passageText + " : " + modification);
}
...
void trash_consultation_note (Webserver_Request& webserver_request, int id)
{
  ...
  database::logs::logv1 (username + " deleted or marked for deletion consultation note " + passageText + " | " + summary + " | " + contents);
}
