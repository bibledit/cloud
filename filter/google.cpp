...
std::pair<std::string, std::string> get_json_key_value_error()
{
    const std::string path = config::logic::google_translate_json_key_path();

    if (not file_or_dir_exists(path))
    {
        return {{}, "Cannot find the JSON key to access Google Translate while looking for file " + path};
    }

    std::string error;
    std::string value = filter_url_file_get_contents(path);
    if (value.empty())
        error = "The key at " + path + " is empty";
...
std::pair<bool, std::string> activate_service_account()
{
    ...
    std::string out_err;
    const int result = filter::shell::run(command.str(), out_err);
    return {(result == 0), out_err};
}


static std::string google_access_token{};
...
std::pair<bool, std::string> print_store_access_token()
{
    ...
    if (success)
        google_access_token = string::trim(out_err);
    else
        google_access_token.clear();
    // Done.
...
void refresh_access_token()
{
    ...
    if (not file_or_dir_exists(config::logic::google_translate_json_key_path()))
        return;

    // Refresh the token.
    const auto [success, token] = print_store_access_token();

    database::logs::logv1("Google access token: " + token);
}
...
std::tuple<bool, std::string, std::string> translate(const std::string& text, const char* source, const char* target)
{
    ...
    std::string translation = filter_url_http_post(url, post_data, {}, error, burst, check_certificate, headers);
    ...
    if (not error.empty())
        database::logs::logv1("Error while translating text: " + error);
...
std::vector<std::pair<std::string, std::string>> get_languages(const std::string& target)
{
    ...
    if (!error.empty())
    {
        database::logs::logv1("Error while getting Google Translate supported languages: " + error);
        language_codes_names.clear();
    }
