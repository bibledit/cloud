...
{
    Assets_Header header(translate("Checks"), webserver_request);
    std::string page = header.run();
    Assets_View view{};


    if (webserver_request.query.contains("approve"))
    {
    ...
    if (webserver_request.query.contains("deleteall"))
    {
    ...
    if (webserver_request.query.contains("deletegroup"))
    {
...

    {
        ...
        for (const database::check::Hit& hit : database::check::get_hits())
        {
            ...
            if (std::ranges::find(bibles, bible) == bibles.cend())
                continue;
            const int id = hit.rowid;
    ...
    // Assemble a block with zero or more fragments of checking results to delete all in one go.
    {
        ...
        const std::set fragments{get_fragments()};
        pugi::xml_document document{};
        using namespace checks::issues;
        ...
        for (int i{start + 1}; i < stop; i++)
        {
            ...
            if (not is_among_fragments(translation))
                continue;
            pugi::xml_node p = document.append_child("p");
            pugi::xml_node a = p.append_child("a");
