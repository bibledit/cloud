...
void sendreceive_bibles ()
{
  if (sendreceive_bibles_watchdog) {
    ...
    if (time < (sendreceive_bibles_watchdog + 900)) {
      database::logs::log<roles::translator> (sendreceive_bibles_text (), translate("Still busy"));
      return;
    }
    database::logs::log<roles::translator> (sendreceive_bibles_text (), translate("Watchdog timeout"));
  }
  sendreceive_bibles_kick_watchdog ();
  config_globals_syncing_bibles = true;
  
  
  database::logs::log<roles::translator> (sendreceive_bibles_sendreceive_text ());
  ...
  if (const int iresponse = filter::string::convert_to_int(response);
    ...
    database::logs::log<roles::translator> (sendreceive_bibles_text (), response);
  ...
  for (std::string bible : bibles) {
    ...
    for (int book : books) {
      ...
      for (int chapter : chapters) {
        ...
        database::logs::log<roles::translator> (sendreceive_bibles_text (), translate("Sending to server"), ":", bible, bookname, chapter);
        ...
        if ((newusfm != oldusfm) && (newusfm != "")) {
          ...
          if (!send_error.empty ()) {
            ...
            database::logs::log<roles::translator> (sendreceive_bibles_text (), "Failure sending chapter:", send_error);
            ...
            communication_errors = true;
            database::logs::log<roles::translator> (sendreceive_bibles_text(), translate ("Failure sending chapter"), "-", translate ("Bibledit Cloud says:"), response);
            // Restore the Bible action for this chapter.
            database::bible_actions::erase (bible, book, chapter);
            database::bible_actions::record (bible, book, chapter, oldusfm);
  ...
  if (communication_errors) {
    database::logs::log<roles::translator> (sendreceive_bibles_text(), "Not downloading changes from the server because of communication errors");
    send_receive_bibles_done ();
    return;
  }
  ...
  if (!error.empty ()) {
    database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Failure getting total checksum"), ":", error);
    send_receive_bibles_done ();
    return;
  }
  if (client_checksum == server_checksum) {
    database::logs::log<roles::translator> (sendreceive_bibles_up_to_date_text());
    send_receive_bibles_done ();
  ...
  if (!error.empty () || v_server_bibles.empty ()) {
    database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Failure getting list of Bibles"));
    send_receive_bibles_done ();
    return;
  }
  ...
  if (server_checksum != message_checksum) {
    database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Checksum error while receiving list of Bibles"));
    send_receive_bibles_done ();
    bible_logic::kick_unreceived_data_timer ();
    return;
  }
  database::logs::log<roles::translator> (sendreceive_bibles_text(), filter::string::implode (v_server_bibles, ", "));
  ...
  if (first_sync_after_connect) {
    ...
    for (std::string bible : bibles) {
      ...
      database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Deleting Bible because the server did not grant access to it"), ":", bible);
    }
  }
  ...
  for (std::string bible : v_server_bibles) {
    ...
    if (!error.empty () || client_checksum_bible.empty ()) {
      database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Failure getting Bible checksum"), ":", error);
      communication_errors = true;
      continue;
    }
    if (client_checksum_bible == server_checksum_bible) {
      database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Bible up to date"), ":", bible);
      continue;
    }
    ...
    if (!error.empty () || server_books.empty ()) {
      database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Failure getting books"), ":", error);
      communication_errors = true;
      continue;
    }
    ...
    if (server_checksum_bible != message_checksum_bible) {
      database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Checksum error while receiving list of books from server"));
      communication_errors = true;
      continue;
    }
    ...
    if (first_sync_after_connect) {
      ...
      if (!client_books.empty ()) {
        ...
        database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Deleting book because the server does not have it"), ":", bible, book_name);
      }
    }
    ...
    for (auto & book : i_server_books) {
      ...
      if (!error.empty ()) {
        database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Failure getting book checksum"), ":", error);
        communication_errors = true;
        continue;
      }
      ...
      if (!error.empty () || server_chapters.empty ()) {
        database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Failure getting list of chapters:"), bible, book_name);
        communication_errors = true;
        continue;
      }
      ...
      if (server_checksum_book != message_checksum_book) {
        database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Checksum error while receiving list of chapters"));
        communication_errors = true;
        continue;
      }
      ...
      if (first_sync_after_connect) {
        ...
        if (!client_chapters.empty ()) {
          ...
          database::logs::log<roles::translator> (sendreceive_bibles_text (), translate("Deleting chapter because the server does not have it"), ":", bible, book_name, chapter);
        }
      }
      ...
      for (auto & chapter : i_server_chapters) {
        ...
        if (!error.empty () || server_checksum_chapter.empty ()) {
          database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Failure getting checksum:"), bible, book_name, chapter);
          communication_errors = true;
          continue;
        }
        ...
        database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Getting chapter:"), bible, book_name, chapter);
        ...
        if (!error.empty () || server_usfm.empty ()) {
          database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Failure getting chapter:"), bible, book_name, chapter);
          communication_errors = true;
          continue;
        }
        ...
        if (checksum_logic::get (server_usfm) != checksum) {
          database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Checksum error while receiving chapter from server:"), bible, book_name, chapter);
          communication_errors = true;
          continue;
        }
        ...
        database::logs::log<roles::translator> (sendreceive_bibles_text(), translate("Merging changes on server and client"), bible, book_name, chapter);
  ...
  // Done.
  database::logs::log<roles::translator> (sendreceive_bibles_text(), "Ready");
  bible_logic::kick_unreceived_data_timer ();
  send_receive_bibles_done ();
}
