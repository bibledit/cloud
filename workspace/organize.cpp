...
std::string workspace_organize(Webserver_Request& webserver_request)
{
    ...
    if (webserver_request.query.count("remove"))
    {
        ...
        if (confirm.empty())
        {
            Dialog_Yes dialog_yes = Dialog_Yes(
                "organize", translate("Would you like to delete this workspace configuration?"));
    ...
    for (size_t i{0}; i < workspaces.size(); i++)
    {
        ...
        add_text(" | ");
        add_operation("?up=" + std::to_string(i), translate("Move workspace up"),
                      filter::string::unicode_black_up_pointing_triangle());
        add_text(" | ");
        add_operation("?down=" + std::to_string(i), translate("Move workspace down"),
                      filter::string::unicode_black_down_pointing_triangle());
        add_text(" | ");
