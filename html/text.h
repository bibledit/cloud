...
{
public:
    explicit HtmlText(const std::string& title);
    ...
    void new_page_break();
    void open_text_style(const stylesv2::Style* style, bool note, bool embed);
    void close_text_style(bool note, bool embed);
    void add_note(const std::string& citation, const std::string& style, bool endnote = false);
    ...
    static pugi::xml_node new_table_row(pugi::xml_node table_element);
    static pugi::xml_node new_table_data(pugi::xml_node table_row_element, bool align_right = false);
