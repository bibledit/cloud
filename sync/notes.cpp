...
std::string sync_notes (Webserver_Request& webserver_request)
{
  ...
  switch (action) {
    ...
    case Sync_Logic::notes_put_summary:
    {
      ...
      database_notes.update_search_fields (identifier);
      // Info.
      database::logs::log<roles::manager> ("Client created or updated a note on the server:", content);
      // Done.
      return std::string();
    }
    ...
    case Sync_Logic::notes_put_comment:
    {
      ...
      database_notes.update_search_fields (identifier);
      // Info.
      database::logs::log<roles::manager> ("Client added comment to note on server:", database_notes.get_summary (identifier));
      // Notifications.
    ...
    case Sync_Logic::notes_put_subscribe:
    {
      ...
      // Info.
      database::logs::log<roles::manager> ("Client subscribed to note on server:", database_notes.get_summary (identifier));
      // Done.
      return std::string();
    }
    case Sync_Logic::notes_put_unsubscribe:
    {
      ...
      // Info.
      database::logs::log<roles::manager> ("Client unsubscribed from note on server:", database_notes.get_summary (identifier));
      // Done.
      return std::string();
    }
    case Sync_Logic::notes_put_assign:
    {
      ...
      // Info
      database::logs::log<roles::manager> ("Client assigned the note to a user on server:", database_notes.get_summary (identifier));
      // Notifications.
    ...
    case Sync_Logic::notes_put_unassign:
    {
      ...
      // Info.
      database::logs::log<roles::manager> ("Client unassigned a user from the note on server:", database_notes.get_summary (identifier));
      // Done.
      return std::string();
    }
    case Sync_Logic::notes_put_status:
    {
      ...
      // Info.
      database::logs::log<roles::manager> ("Client set the note status on server:", database_notes.get_summary (identifier));
      // Done.
      return std::string();
    }
    ...
    case Sync_Logic::notes_put_severity:
    {
      ...
      // Info
      database::logs::log<roles::manager> ("Client set the severity for a note on server:", database_notes.get_summary (identifier));
      // Done.
      return std::string();
    }
    ...
    case Sync_Logic::notes_put_mark_delete:
    {
      ...
      // Info.
      database::logs::log<roles::manager> ("Client marked a note on server for deletion:", database_notes.get_summary (identifier));
      // Notifications.
    ...
    case Sync_Logic::notes_put_unmark_delete:
    {
      ...
      // Info.
      database::logs::log<roles::manager> ("Client unmarked a note on server for deletion:", database_notes.get_summary (identifier));
      // Done.
      return std::string();
    }
    case Sync_Logic::notes_put_delete:
    {
      ...
      database::logs::log<roles::manager> ("Client deleted a note on server:", database_notes.get_summary (identifier));
      // Notifications.
