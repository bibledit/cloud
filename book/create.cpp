                 ...
                 std::vector<std::string>& feedback)
{
    ...
    const auto book_filter = [book](const auto& row) noexcept
    {
        return book == static_cast<book_id>(row.book());
    };
    for (const auto& row : versification_data | std::views::filter(book_filter))
    {
        const int ch = row.chapter();
        const int verse = filter::string::convert_to_int(row.verse());
    ...
    const std::string created = std::accumulate(chapters_created.cbegin(), chapters_created.cend(), std::string(),
                                                filter);
