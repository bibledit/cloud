...
std::string editone_update(Webserver_Request& webserver_request)
{
    ...
    if (good2go)
    {
        if (!filter::string::unicode_string_is_valid(loaded_html) || !
            filter::string::unicode_string_is_valid(edited_html))
        {
    ...
    if (good2go && bible_write_access && text_was_edited)
    {
        if (loaded_verse_usfm != existing_verse_usfm)
        {
            ...
            std::string merged_verse_usfm = filter_merge_run(loaded_verse_usfm, edited_verse_usfm, existing_verse_usfm,
                                                             true, conflicts);
            // Mail the user if there is a merge anomaly.
            filter_merge_add_book_chapter(conflicts, book, chapter);
            bible_logic::optional_merge_irregularity_email(bible, book, chapter, username, loaded_verse_usfm,
    ...
    if (good2go && bible_write_access && text_was_edited)
    {
        message = filter::usfm::safely_store_verse(webserver_request, bible, book, chapter, verse, edited_verse_usfm,
                                                   explanation, true);
    ...
    if (good2go && bible_write_access && text_was_edited)
    {
        ...
        if (message.empty())
        {
            ...
            if constexpr (config::logic::platform() == config::logic::Platform::cloud)
            {
                database::modifications::recordUserSave(username, bible, book, chapter, old_id, old_chapter_usfm, newID, new_chapter_usfm);
