...
std::string zip_folder_miniz_internal (std::string folder)
{
  ...
  for (auto path : paths) {
    ...
    if (!status) {
      database::logs::logv1 ("mz_zip_add_mem_to_archive_file_in_place failed for " + path);
      return std::string();
    }
  }
  return zippedfile;
...
std::string unzip_miniz_internal (std::string zipfile)
{
  ...
  if (status) {
    ...
    for (unsigned i = 0; i < filecount; i++) {
      ...
      if (status) {
        ...
        if (mz_zip_reader_is_file_a_directory (&zip_archive, i)) {
          ...
          if (!status) {
            database::logs::logv1 ("mz_zip_reader_extract_to_file failure for file " + filename + " in " + zipfile);
            error = true;
          }
        }
        
      } else {
        database::logs::logv1 ("mz_zip_reader_file_stat failed for " + zipfile);
        error = true;
      }
    }
    ...
    database::logs::logv1 ("mz_zip_reader_init_file failed for " + zipfile);
    error = true;
  }
