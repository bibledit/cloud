...
bool editone_index_acl(Webserver_Request& webserver_request)
{
    ...
    if (constexpr int minimum_role = roles::translator;
        roles::access_control(webserver_request, minimum_role))
        return true;
...
{
    if (webserver_request.query.contains("switchbook") and webserver_request.query.contains("switchchapter"))
    {
        const int switch_book = filter::string::convert_to_int(webserver_request.query["switchbook"]);
        const int switch_chapter = filter::string::convert_to_int(webserver_request.query["switchchapter"]);
        ipc_focus::set_passage(webserver_request, switch_book, switch_chapter, 1);
        navigation_passage::record_history(webserver_request, switch_book, switch_chapter, 1);
    }

    Assets_Header header (translate("Edit verse"), webserver_request);
    ...
    if (webserver_request.query.contains("bible"))
        bible = access_bible::clamp(webserver_request, webserver_request.query["bible"]);
    {
        constexpr auto identification{"bibleselect"};
    ...
    script_stream << "var verseSeparator = " << std::quoted(database::config::general::get_notes_verse_separator()) <<
        ";\n";
    std::string script{std::move(script_stream).str()};
    ...
    const int letter_spacing = database::config::bible::get_letter_spacing(bible);
    ...
    view.set_variable("spellcheck",
                      filter::string::convert_to_true_false(
                          webserver_request.database_config_user()->get_enable_spell_check()));
